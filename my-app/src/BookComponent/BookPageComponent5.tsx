...
const BookPageComponent5: React.FC<NewComponentProps> = () => {
  ...
  const [pagesRead, setPagesRead] = useState(
    parseInt(localStorage.getItem("Component5 - pagesRead") || "0", 10)
  );
  const [isAvailable, setIsAvailable] = useState(
    localStorage.getItem("Component5 - isAvailable") === "true"
  );

  //useEffect that is used for saving the user input, and if the book is available on the bookstore
  useEffect(() => {
    localStorage.setItem("Component5 - pagesRead", pagesRead.toString());
    localStorage.setItem("Component5 - isAvailable", isAvailable.toString());
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await booksInfo.get("http://localhost:3000/data/5");
        setInfo([res.data]);
...
export default BookPageComponent5;
